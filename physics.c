...
int main(void)
{
	...
	for (float i = 0.0f; i <= screen_height + 1;
	     i += screen_height / 10.0f) {
	...
	for (int i = 0; i < y_count; ++i) {
		...
		rotated_y_lines[i] = vec2_linear_translate_line(
			temp_l, (T_Matrix){ .x1 = 0.0f,
					    .y1 = 1.0f,
					    .x2 = -1.0f,
					    .y2 = 0.0f });
	...
	screen_to_world(&translated_origin, &(Vector2){
						    origin_rec.x,
						    origin_rec.y,
					    });
